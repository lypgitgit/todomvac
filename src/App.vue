<template>
  <div id="app">
    <h1>todos</h1>
    <label class="taskLable" @click="allTask"></label>
    <input autofocus="autofocus" autocomplete="off"  placeholder="What needs to be done?" class="newTodo"  v-on:keyup.enter="nextTask"   v-model="onceRow" >
    <router-view/>
    <footer class="footer" v-show="state">
        <span class="rowCount" >{{completed_count}}<strong> item left</strong></span>
        <ul class="foot_nav">
        ...
        <span class="clearTask"  @click='delCompleted'>
...
export default {
  ...
  methods: {
    showNav: function () {
      if (this.rowsValue.length > 0) { this.state = true } else { this.state = false }
    },
    nextTask: function () {
      var that = this
      console.log(that.onceRow)
      if (that.onceRow) {
        that.$store.commit('SET_ROWDATA', {content: that.onceRow, state: false})
        that.onceRow = ''
        that.updateDate()
    ...
    editContent: function (that) {
      const currNode = that.currentTarget
